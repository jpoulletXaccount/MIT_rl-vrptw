...
from evaluation.useful_stops import manager_stop_tw,route
...
class RoutingEvaluator(object):
    ...
    def _update_filename(self,benchmark_type):
        ...
        if self.args['ups']:
            task_name = 'vrptw-ups-size-{}-len-{}-results-{}.txt'.format(self.args['test_size'], self.env.n_nodes,benchmark_type)
        else:
    ...
    def kmean_clustering(manager_stop,number_cluster):
        ...
        for list_stop in dict_cluster_stops.values():
            final_list_manager.append(manager_stop_tw.ManagerStopTW.from_stops_list(manager_stop,list_stop))
