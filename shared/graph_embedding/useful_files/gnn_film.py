...
def sparse_gnn_film_layer_with_dist(node_embeddings: tf.Tensor,
    ...
    """
    Almost the same as below, but take the distance into acount in the weight
    """
    proba = True
    ...
    # Let M be the number of messages (sum of all E):
    message_targets = tf.cast(tf.concat(edge_type_to_message_targets, axis=0),dtype=tf.int32)  # Shape [M]
    ...
    for _ in range(num_timesteps):
        ...
        for edge_type_idx, adjacency_list_for_edge_type in enumerate(adjacency_lists):
            ...
            if proba:
                modulated_messages = dist_between_edges * (per_message_film_gamma_weights * messages + per_message_film_beta_weights)
            else:
                modulated_messages = dist_between_edges * per_message_film_gamma_weights * messages + per_message_film_beta_weights
            messages_per_type.append(modulated_messages)
